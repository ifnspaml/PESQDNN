#####################################################################################
# Inference_PESQ_DNN_FLE:
# This script used to predict the coded speech PESQ score employing the non-intrusive PESQ-DNN model trained employing frame-level embeddings (FLE)
# This model is trained with loss explicilty control FLE scores, as defined in (4). Details please check the paper. 
#
# Given data :
...
from keras.layers import Input, Add, Multiply, Average, Activation, Concatenate, LeakyReLU, Flatten, Permute, TimeDistributed, Bidirectional, Masking, ConvLSTM2D, Reshape, Lambda, Dense, LSTM
...
block_out2 = TimeDistributed(Dense(32, activation='relu'))(lstm1)

block_out = TimeDistributed(Dense(16))(block_out2)

Mos_predicted_block = TimeDistributed(Lambda(output_layer))(block_out)

avg_out = Lambda(k_mean_layer)(Mos_predicted_block)
